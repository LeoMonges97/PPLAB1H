...
/**@brief avi_altaConIndice(): Realiza el alta de una entidad solo si el indice que le paso corresponde a un Empty.
 * @param Aviso* pArray, Es el puntero al array de enteros.
 * @param int limite, es el limite de array.
 * @param int indice, es el indice donde se cargara el ent.
 * @return (-1) Error / (0) Ok
 */
int avi_altaConIndice(Aviso* pArray,int limite,int indice)
{
	int retorno = -1;
	Aviso buffer;
...
int avi_modificar(Aviso* pArray,int limite,Cliente* pArrayCliente,int limiteCliente)
{
	...
	avi_imprimir(pArray,limite);

	if(utn_getInt("\n ~ Ingrese el ID a modificar: "," > Error!\n", &idModificar,5,1,1000) == 0)
	{
		if(avi_buscarId(pArray, limite, &indice, idModificar) == 0)
		{
			printf("\n . Dato seleccionado:\n");
	...
	if(utn_getInt("\n ~ Ingrese el ID del aviso a modificar: "," > Error!\n", &idModificar,5,1,1000) == 0)
	{
		...
		{
			printf("\n . Dato seleccionado:\n");
			cli_imprimirPorID(pArrayCliente,limiteCliente,indice); // mas imprimir el cliente
 ...
 * @return (-1) Error / (0) Ok
 */
int avi_baja(Aviso* pArray, int limite)
{
	// Tipo de baja: Logica
	int retorno = -1;
	int idBajar;
	...
	if(utn_getInt("\n ~ Ingrese el ID del Aviso a bajar: "," > Error!\n", &idBajar,2,1,1000) == 0)
	{
		if(avi_buscarId(pArray, limite, &indice, idBajar) == 0)
		{
			printf("\n . Dato seleccionado:\n");
			avi_imprimirPorID(pArray,limite,indice);
