...
static int avi_borrarCadaAvisoDelCliente(Aviso* pArray,int limite,int idAviso);
static int avi_clienteConAvisosActivos(Aviso* pArray,int limite,int idAInvestigar);
static int avi_borrarCadaAvisoDelCliente(Aviso* pArray,int limite,int idAviso)
{
	int i;
	int retorno = -1;

	if(pArray != NULL && limite > 0 && idAviso > 0)
	{
		for(i = 0;i < limite; i++)
		{
			if(pArray[i].idCliente == idAviso)
			{
...
static int avi_clienteConAvisosActivos(Aviso* pArray,int limite,int idAInvestigar)
{
	...
	for(i = 0; i < limite; i++)
	{
		if(pArray[i].idCliente == idAInvestigar &&
...
int avi_contarCantidadDeAvisosPorCliente(Aviso* pArray, int limite, int idCliente)
{
	int retorno = -1;
	int i;
	int cantAvisos = 0;
	...
	for(i=0; i < limite; i++)
	{
		if(pArray[i].estado == FALSE && pArray[i].idCliente == idCliente)
		{
			cantAvisos++;
			retorno  = cantAvisos;
		}
	...
	if(pArray != NULL && limite > 0 && idDelCliente > 0)
	{
		for(int i = 0; i < limite; i++)
		{
			if(pArray[i].isEmpty == FALSE && pArray[i].idCliente == idDelCliente)
			{
				...
				cantidadAvisosActivos = avi_clienteConAvisosActivos(pArray,limite,idDelCliente);
	...
	if(pArray != NULL && limite > 0 && pArrayCliente != NULL && limiteCliente > 0)
	{
		for(indice = 0; indice < limiteCliente; indice++)
		{
			if(pArrayCliente[indice].isEmpty == FALSE)
			{
				...
				avi_avisosDeUnCliente(pArray, limite,pArrayCliente[indice].id);
			}
		}
		retorno = 0;
	}
 ...
 * @return (-1) Error / (0) Ok
 */
int avi_altaForzada(Aviso* pArray, int limite, int indice, int id,
		int estado,
		char* texto,
		int idCliente,
		int numeroRubro
...
int avi_baja(Aviso* pArray, int limite,Cliente* pArrayCliente,int limiteCliente, int idSeleccionado)
{
	...
	avi_avisosDeUnCliente(pArray,limite,idSeleccionado);
